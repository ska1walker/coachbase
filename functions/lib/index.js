...
exports.acceptInvite = exports.createInvite = exports.setAdminRole = exports.saveMatchHistory = exports.adminResetUserPassword = void 0;
...
exports.adminResetUserPassword = functions.https.onCall(async (data, context) => {
    ...
    if (!targetUid || typeof targetUid !== 'string') {
        throw new functions.https.HttpsError('invalid-argument', 'targetUid must be a valid string');
    }
 ...
 */
exports.saveMatchHistory = functions.https.onCall(async (data, context) => {
    ...
    if (!squadId || !teams || !Array.isArray(teams)) {
        throw new functions.https.HttpsError('invalid-argument', 'Invalid match history data');
    }
    try {
        ...
        if (!isOwner && !isCoTrainer) {
            throw new functions.https.HttpsError('permission-denied', 'You can only save history for squads you own or co-train');
        }
        ...
        throw new functions.https.HttpsError('internal', `Failed to save match history: ${error.message}`);
    }
});
/**
...
exports.createInvite = functions.https.onCall(async (data, context) => {
    ...
    if (!squadId || typeof squadId !== 'string') {
        throw new functions.https.HttpsError('invalid-argument', 'squadId must be a valid string');
    }
    try {
        ...
        if (squadData?.ownerId !== context.auth.uid) {
            throw new functions.https.HttpsError('permission-denied', 'Only squad owners can create invites');
        }
        // Get caller's email
        ...
        throw new functions.https.HttpsError('internal', `Failed to create invite: ${error.message}`);
    }
});
/**
...
exports.acceptInvite = functions.https.onCall(async (data, context) => {
    // Check authentication
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated to accept invites');
    }
    const { token } = data;
    if (!token || typeof token !== 'string') {
        throw new functions.https.HttpsError('invalid-argument', 'token must be a valid string');
    }
    try {
        ...
        const squadData = squadDoc.data();
        if (squadData?.ownerId === context.auth.uid) {
            throw new functions.https.HttpsError('failed-precondition', 'You are already the owner of this squad');
        }
        ...
        if (currentCoTrainers.includes(context.auth.uid)) {
            throw new functions.https.HttpsError('failed-precondition', 'You are already a co-trainer for this squad');
        }
        ...
        throw new functions.https.HttpsError('internal', `Failed to accept invite: ${error.message}`);
    }
});
